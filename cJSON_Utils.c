...
static int cJSONUtils_Compare(cJSON *a, cJSON *b)
{
    ...
    switch (a->type & 0xFF)
    {
        case cJSON_Number:
            /* numeric mismatch. */
            return (a->value.number != b->value.number) ? -2 : 0;
        case cJSON_String:
            /* string mismatch. */
            return (strcmp(a->value.string, b->value.string) != 0) ? -3 : 0;
        case cJSON_Array:
...
static int cJSONUtils_ApplyPatch(cJSON *object, cJSON *patch)
{
    ...
    /* decode operation */
    if (!strcmp(op->value.string, "add"))
    {
        opcode = 0;
    }
    else if (!strcmp(op->value.string, "remove"))
    {
        opcode = 1;
    }
    else if (!strcmp(op->value.string, "replace"))
    {
        opcode = 2;
    }
    else if (!strcmp(op->value.string, "move"))
    {
        opcode = 3;
    }
    else if (!strcmp(op->value.string, "copy"))
    {
        opcode = 4;
    }
    else if (!strcmp(op->value.string, "test"))
    {
        /* compare value: {...} with the given path */
        return cJSONUtils_Compare(cJSONUtils_GetPointer(object, path->value.string), cJSON_GetObjectItem(patch, "value"));
    }
    else
    {
        /* unknown opcode. */
    ...
    {
        /* Get rid of old. */
        cJSON_Delete(cJSONUtils_PatchDetach(object, (unsigned char*)path->value.string));
        if (opcode == 1)
        {
    ...
    if ((opcode == 3) || (opcode == 4))
    {
        ...
        {
            /* move */
            value = cJSONUtils_PatchDetach(object, (unsigned char*)from->value.string);
        }
        if (opcode == 4)
        {
            /* copy */
            value = cJSONUtils_GetPointer(object, from->value.string);
        }
        if (!value)
        {
            /* missing "from" for copy/move. */
    ...
    parentptr = cJSONUtils_strdup((unsigned char*)path->value.string);
...
static void cJSONUtils_CompareToPatch(cJSON *patches, const unsigned char *path, cJSON *from, cJSON *to)
{
    ...
    {
        case cJSON_Number:
            if (from->value.number != to->value.number)
            {
                cJSONUtils_GeneratePatch(patches, (const unsigned char*)"replace", path, 0, to);
            }
            return;
        ...
        case cJSON_String:
            if (strcmp(from->value.string, to->value.string) != 0)
            {
                cJSONUtils_GeneratePatch(patches, (const unsigned char*)"replace", path, 0, to);
            }
            return;
